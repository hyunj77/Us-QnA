...
export default function QnAQuestionList() {
  ...
  let shown = activeSub === '전체' ? questions : questions.filter((q) => q.subcategory === activeSub)
  if (statusFilter === 'answered') shown = shown.filter((q) => answeredIds.has(q.id))
  if (statusFilter === 'unanswered') shown = shown.filter((q) => !answeredIds.has(q.id))
  if (statusFilter === 'bookmarked') {
    const bookmarked = new Set(getBookmarkedIds())
    shown = shown.filter((q) => bookmarked.has(q.id))
  }
  ...
  const goRandom = () => {
    ...
    if (q) navigate(`/qna/${categoryId}/${q.id}`)
  }
  ...
  return (
    <div className="screen">
      <TopAppBar
        ...
        right={
          <>
            <button className="topbar-icon-btn" onClick={goRandom} aria-label="랜덤 질문"><Shuffle size={19} /></button>
            <button className="topbar-icon-btn" onClick={() => navigate('/qna/search')}><Search size={20} /></button>
          </>
        }
      />
      ...
      <div className="q-list-filter-row" style={{ padding: '0 20px 12px' }}>
        {STATUS_FILTERS.map((f) => (
          <button
            key={f.value}
            className={`q-list-filter-btn ${statusFilter === f.value ? 'q-list-filter-btn-active' : ''}`}
